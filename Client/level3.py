...
def add():
    ...
    if sf.filename:
        add_file(sf.filename,level_path,3)
...
def enter_level3():
    ...
    #Create an Entry Widget in the Toplevel window
    title= Label(top, bg = '#ffbf00', text="Face validation is required to enter Level 2")
    title.pack()
    Info= Label(top, bg = '#ffbf00', text="Info: Press 'q' to exit anytime during the validation!")
    Info.pack()
    #Create a Button Widget in the Toplevel Window
...
sf = Tk()
sf.geometry('700x480')
sf.title('Safehouse_level3')
sf['bg']='#BB9981'
level_path = "level1/level2/level3"
f = ("Times bold", 14)
...
# button_edit = Button(footer_frame, text = "Level 3", command = enter_level3)
...
unencrypted_files_list, encrypted_files_list = show_all_files(3)
