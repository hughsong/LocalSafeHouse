...
def close_win_face_reg(top):
    if check_face():
        top.destroy()
        sf.destroy()
        import level2
    else:
        messagebox.showerror("Failed verification!", "Fail the face recognition")
        top.destroy()
...
def add():
    ...
    if sf.filename:
        add_file(sf.filename,level_path,1)
...
def enter_level2():
    ...
    top.geometry("300x250")
    #Create an Entry Widget in the Toplevel window
    label1= Label(top , text="Face validation is required to enter Level 2")
    label1.pack(pady = 20)
    label2= Label(top , text="Press 'ok' button to continue")
    label2.pack(pady = 20)
    Info= Label(top, text="Press 'q' to exit anytime during the validation!")
    Info.pack(pady = 20)
    #Create a Button Widget in the Toplevel Window
    button= Button(top, text="OK", command=lambda:close_win_face_reg(top))
    button.pack(pady=5, side= TOP)
...
sf = Tk()
sf.geometry('700x480')
sf.title('Safehouse_level1')
sf['bg']='#BB9981'
level_path = "level1"
f = ("Times bold", 14)
#ffbf00
footer_frame = Frame(sf,bg='#BB9981',height=45)
...
button_edit = Button(footer_frame, text = "Level 2",  height = 2,width=7, borderwidth=3, activebackground='#345', relief="ridge", command = enter_level2)
...
unencrypted_files_list, encrypted_files_list = show_all_files(1)
